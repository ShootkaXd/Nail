        ...
        <nav className="flex-1 p-4 space-y-1">
          {nav.map(({ to, label, Icon }) => (
            <NavLink key={to} to={to}
              className={({ isActive }) =>
                `relative flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 ${isActive ? 'bg-brand-50 text-brand-600 translate-x-0.5' : 'text-gray-600 hover:bg-gray-100'}`
              }
            >
              {({ isActive }) => (
                <>
